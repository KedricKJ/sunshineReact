...
class BranchPage extends Component {
  ...
  handleModalSubmit = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      if (!id) {
        axios.post("/branches", fieldsValue, { headers }).then(() => {
          form.resetFields();
          this.setState({ modalVisible: false });
          this.handleRefresh();
        });
      } else {
        axios.put("/branches/" + id, fieldsValue, { headers }).then(() => {
          form.resetFields();
          this.setState({ modalVisible: false });
          this.handleRefresh();
        });
  ...
  handleDelete = id => {
    axios.delete("/branches/" + id, { headers }).then(() => {
      this.handleRefresh();
    });
  };
  ...
  render() {
    ...
    const modalProps = {
      ...
      maskClosable: false,
      title:
        this.state.modalType === "create" ? "Create Branch" : "Update Branch",
      centered: true
    };
    const getModalContent = () => {
      return (
        <Form>
          <FormItem label="Branch Name" hasFeedback {...this.formLayout}>
...
export default Form.create()(BranchPage);
